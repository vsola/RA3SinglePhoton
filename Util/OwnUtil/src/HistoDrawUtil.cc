...
double roundDouble(double xin, int n ) {
	double d = pow(10.0, n);
...
string getStringFromDouble(double i, int n ) {
	//---------
	if(i<0.05&&n==1)n=n+1;
	i = roundDouble(i,n);
	char result[100];
...
string getStringFromDoubleMin(double i, int n ) {
	//---------
	if(i>=10)n=0;
...
void drawCmsPrel(double intLumi, string METCut, bool onlyChannelInfo, int noJets, bool isBestjet, string jetLabel, bool drawChannelInfo, bool drawCMS , bool drawLumiOnly, bool largerMarginRight) {
...
void cmsPrel(double intLumi , double pos , bool isData,bool isMC) {
	TLatex latex;
	latex.SetNDC();
...
void writeStatusBar(int totEvents, double currEvent, int nSteps) {
