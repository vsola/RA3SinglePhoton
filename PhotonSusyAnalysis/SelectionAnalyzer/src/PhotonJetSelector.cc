   ...
   eeBadScFilterSrc_(params.getParameter<edm::InputTag> ("trackingFailureFilterSrc"))
{
  ...
  for (int cutIt = 0; cutIt < (int) (allcuts.size()); ++cutIt) {
    ...
    } else if (cut == "photonPtCor") {
      push_back(cut, photonPtCorMin);
    } else if (cut == "photon2Pt") {
      push_back(cut, photon2PtMin);
  ...
  photonPtCor_  = index_type(&bits_, std::string("photonPtCor"));
...
double PhotonJetSelector::getEtRelPhotonJets() {
  for (std::vector<pat::Jet>::const_iterator jetsBegin = allPhotonJets_.begin(), jetsEnd = allPhotonJets_.end(), ijet = jetsBegin; ijet != jetsEnd; ++ijet) {
    if ( isJetBetterPhoton(*ijet, *photon_) ){
...
bool PhotonJetSelector::operator ()(const edm::EventBase & event, pat::strbitset & ret) {
	    ...
	    selectedPhotonsHLT_ = getCleanPhotonsHLT(photonsHLT); //HLT PHOTONS (eta selected)
		...
		*/
		...
		if (!pass && photon_ != 0) {
		  ...
		  if ( ( !eventHasMuon && !eventHasElectron ) || ( electrons->size() == 0 && muons->size() == 0 ) ) {
	    ...
	    if (allCutsPassed) {
	      bool pass = !considerCut( photonPtCor_ );
	      if ( photon_ != 0 ) {
		// if (photon_->pt() > cut(photonPtCor_, double())) {
		if ( photon_->userFloat("ptMatchedJet") > cut( photonPtCor_, double() ) ) {
		  passCut( ret, photonPtCor_ );
		  pass = true;
		}
	      }
	      allCutsPassed = allCutsPassed && pass;
	    ...
	    //2nd photon pt
	    if (allCutsPassed) {
	      bool pass = !considerCut(photon2Pt_);
	      if (photon2_ != 0) {
		if (photon2_->pt() > cut(photon2Pt_, double())) {
		  passCut(ret, photon2Pt_);
		  pass = true;
		}
	      }
	      allCutsPassed = allCutsPassed && pass;
	    ...
	    //cut on photon-jet matching
	    if ( allCutsPassed && cut(photonJetMatch_, double()) != 0 && isPhotonValid() ) {
	      bool pass = !considerCut(photonJetMatch_);

	      if (!pass) {
		bool hasMatchedJet = false;

		if ( selectedPhotonJets_.size() > 0 ) {
		  for (std::vector<pat::Jet>::const_iterator jetsBegin = selectedPhotonJets_.begin(), 
			 jetsEnd = selectedPhotonJets_.begin() + 1, ijet = jetsBegin; 
		       ijet != jetsEnd; ++ijet) { //selected photon jets

		    if ( isJetBetterPhoton(*ijet, *photon_) ){
		      hasMatchedJet = true;

		    }//isJetBetterPhoton
		  }//for selectedPhotonJets
		}//if selectedPhotonJets > 0

		if ( hasMatchedJet && cut(photonJetMatch_, double()) == 1 ) {
		  passCut(ret, photonJetMatch_);
		  pass = true;
		} else if ( !hasMatchedJet && cut(photonJetMatch_, double()) == 2 ) {
		  passCut(ret, photonJetMatch_);
		  pass = true;
		}
	    ...
	    if ( allCutsPassed && cut( photonTightMatch_, double()) != 0 && isPhotonValid() ) {
	      ...
	      double photonHTLpt = 0.;

	      if ( selectedPhotonsHLT_.size() > 0 ) {
		for ( std::vector<pat::Photon>::const_iterator photonBegin = selectedPhotonsHLT_.begin(), 
			photonEnd = selectedPhotonsHLT_.begin() + 1, iphoton = photonBegin; iphoton != photonEnd; ++iphoton ) { 
		  // only 1st HLT photon
		  if ( selectedJetsHLTAll_.size() > 0 ) {
		    for ( std::vector<pat::Jet>::const_iterator jetsBegin = selectedJetsHLTAll_.begin(), 
			    jetsEnd = selectedJetsHLTAll_.end(), ijet = jetsBegin; ijet != jetsEnd; ++ijet ) {

		      if ( isJetBetterPhoton(*ijet, *iphoton) ){
			if (ijet->jecSetsAvailable() ) 
			  photonHTLpt = ijet->correctedJet( getPhotonJECLevel(*ijet) ).pt() ;
			else
			  photonHTLpt = ijet->pt();

		      } else {
			photonHTLpt = iphoton->pt();
		      }//isJetBetterPhoton
		      
		    }//for selectedJetsHLTAll
		  }//if selectedJetsHLTAll > 0
		}//for selectedPhotonsHLT
	      }//if selectedPhotonsHLT > 0


	      if ( selectedPhotonsHLT_.size() == 0 || photonHTLpt < 110. ) {		
...
void PhotonJetSelector::crosscleanSelectedPhotonsAndJets(bool applyPhotonIso) {
  ...
  selectedPhotonJets_ = crosscleanedPhotonJets; // deltaR <  0.2, ETrel < 3.
  ...
  if ( selectedPhotons_.size() > 0 ) {
	...
	if ( isJetBetterPhoton(*ijet, *iphoton) ) { //deltaR < 0.2, pTrel = 0.8 - 3
	  ...
	  pat::Jet corrJetL1 = ijet->correctedJet("L1FastJet"); //knut
	...
	pat::Photon p = *iphoton;
	p.addUserFloat("ptMatchedJet", p.pt());
...
//
std::vector<pat::Photon> PhotonJetSelector::getCleanPhotonsHLT(edm::Handle<std::vector<pat::Photon> > photons) {
  ...
  for (std::vector<pat::Photon>::const_iterator photonBegin = photons->begin(), 
    ...
    if ( abs(iphoton->eta()) < cut( photonEta_, double() ) && ( iphoton->pt() > 40. ) ) {
      passPhotonEta = true;
    }//if eta
